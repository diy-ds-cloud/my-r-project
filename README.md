...
1. [Binder](https://mybinder.org) can start a customized computing environment in the browser with software requirements pre-installed.  
    ...
    [![Binder](https://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/diy-ds-cloud/my-r-project/main?urlpath=git-pull%3Frepo%3Dhttps%253A%252F%252Fgithub.com%252Fdiy-ds-cloud%252Fmy-r-project%26urlpath%3Dlab%252Ftree%252Fmy-r-project%252F%26branch%3Dmain)
1. With `docker` and `docker-compose` available, start dev container:
    ```bash
    docker-compose build jupyterlab
    docker-compose up
    ```
1. Start a [terminal session in Jupyter Lab](https://jupyterlab.readthedocs.io/en/stable/user/terminal.html) and `cd` into directory `my-r-project`
    ```bash
    cd ~/my-r-project
    ```
3. In a terminal prompt, execute  
    ```bash
    ./diy-my-r-project config
    ```
...
1. Subsequently, execute 
    ```bash
    ./diy-my-r-project deploy
    ```

### Step 3: Open and start interface session (Jupyter notebook or RStudio)
1. Find and open a URL on the last line of `diy-my-r-project deploy` command. Something like,
    ```bash
...
1. `chartpress.yaml` file and `scripts` directory contain information needed for [`chartpress`](https://github.com/jupyterhub/chartpress). Chartpress does the following:  
    a. Compile a Helm chart and Docker images with [Github actions](https://github.com/diy-ds-cloud/my-r-project/actions)  
    b. Update the [project repository page](https://diy-ds-cloud.github.io/my-r-project)  
...
### `diy-my-r-project` script

`diy-my-r-project` script coordinates the configuration and deployment of infrastructure on the cloud with the above functionalities.

1. `diy-my-r-project config` command saves Google cloud account, project id, compute zone as well as Helm chart version to use in deployment.
1. `diy-my-r-project deploy` command executes terraform and helm commands to deploy Kubernetes cluster and populates "pods".
